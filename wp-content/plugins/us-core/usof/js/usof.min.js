...
return self}}}(jQuery);var _window=window,_document=document,_undefined=undefined;['$ush','$usof'].map(function(name){_window[name]=_window[name]||{}});jQuery.fn.usMod=function(mod,value){if(this.length==0){return this}
...
$usof.mixins.Events={on:function(eventType,handler){var self=this;if($ush.isUndefined(self.$$events)){self.$$events={}}(eventType+'').split(' ').map(function(_eventType){if($ush.isUndefined(self.$$events[_eventType])){self.$$events[_eventType]=[]}
...
self._events.change=function(){self.trigger('change',[self.getValue()])};self.$input.on('change',self._events.change);return self},isEditLive:function(){var self=this;return!!(self.$row.closest('.usb-panel-fieldset').length||self.$row.parents('.usb-panel-body').length)},initResponsive:function(){var self=this;if(!self.hasResponsive()){return}
...
return!1},isObjectValue:function(value){return value&&(''+value).indexOf($ush.rawurlencode('{'))===0},isValidState:function(state){return state&&(this._states||[]).indexOf(''+state)!==-1},isVCParamValue:function(){return this.$input.hasClass('wpb_vc_param_value')},getParent:$.noop,getRelatedField:function(){var self=this,parent=self.getParent();if(!$ush.isUndefined(self.relatedOn)&&parent instanceof $usof.GroupParams){return(parent.fields||{})[$ush.toString(self.relatedOn)]}
return _undefined},getCurrentState:function(){var self=this;if(!self.isValidState(self._currentState)){self._currentState='default'}
...
self.$input.val(value);self.trigger('change',value);if(self.isVCParamValue()&&self.$input.is(':hidden')){self.$input.trigger('change')}},getValue:function(){return this.$input.val()},setValue:function(value,quiet){var self=this;if(!self.isResponsive()&&self.isResponsiveValue(value)){self.$row.addClass('responsive')}
...
self.groupParams=[];$('.usof-form-group-item',self.$field).each(function(i,groupParams){var $groupParams=$(groupParams);if(!$groupParams.parent().hasClass('usof-form-group-prototype')){$groupParams.remove()}});$.each(value,function(index,paramsValues){var groupPrototype=self.$groupPrototype.html();if(self.$btnAddGroup.length){self.$btnAddGroup.before(groupPrototype)}else{self.$field.append(groupPrototype)}
var $groupParams=$('.usof-form-group-item',self.$field).last();var groupParams=new $usof.GroupParams($groupParams);groupParams.setValues(paramsValues,1);for(var k in groupParams.fields){if(!groupParams.fields.hasOwnProperty(k)){continue}
groupParams.fields[k].trigger('change');break}});self._reInitParams()},getValue:function(){var self=this,result=[];$.each(self.groupParams,function(i,groupParams){result.push(groupParams.getValues())});if(self.hasStringValue){if(result.length){try{result=$ush.toString(result)}catch(err){console.error(result,err);result=self.getDefaultValue()}}else{result=self.getDefaultValue()}}
return result},addGroup:function(index){this.$btnAddGroup.addClass('adding');var $groupPrototype=$(this.$groupPrototype.html());if(this.isForButtons&&!$ush.isUndefined(index)){this.$btnAddGroup.closest('.usof-form-group').find(' > .usof-form-group-item:eq('+parseInt(index)+')').after($groupPrototype)}else{this.$btnAddGroup.before($groupPrototype)}
var groupParams=new $usof.GroupParams($groupPrototype);for(var k in groupParams.fields){groupParams.fields[k].on('change',this._events.changeGroupParams)}
if(this.isForButtons&&index!==_undefined){this.groupParams.splice(index+1,0,groupParams)}else{this.groupParams.push(groupParams)}
...
do{newIndexIsUnique=!0;for(var i in this.groupParams){if(this.groupParams[i].fields.name.getValue()==this.groupTranslations.style+' '+newIndex){newIndex ++;newIndexIsUnique=!1;break}}}while(!newIndexIsUnique);groupParams.fields.name.setValue(this.groupTranslations.style+' '+newIndex);groupParams.fields.id.setValue(newId);groupParams.$container.find('.usof-btn-class-main').text('us-btn-style_'+newId)}
...
return!1};$.fn.usPopup=function(){this.each(function(){this.data('USOF_Popup',new $usof.popup(this.data('popup-id')))})}})(jQuery);!function($,undefined){"use strict";var _document=document,_parseFloat=parseFloat,_parseInt=parseInt,_undefined=undefined,_window=window;var USColorPicker={init:function(options){var self=this;if(!!self._inited){return}
self._inited=!0;self.$window=$(_window);self.$document=$(_document);self.value=options.value;var defaults={state:'solid',height:160,width:160,angle:220,inputHeight:options.input.height(),initialSecondColor:{hex:'#000000',rgba:{r:0,g:0,b:0,a:1},hsba:{h:360,s:0,b:0,a:1,},},onChange:$.noop,color:{first:{},second:{},},initialColor:self.value,};self.sending=!1;self.colors=$.extend({},defaults,options||{});self.$input=options.input;self.withGradient=self.$input.closest('.usof-color').hasClass('with-gradient');self.$colpickTemplate=$('.usof-colpick.usof-colpick-template:first');$('.usof-colpick:not(.usof-colpick-template)').remove();self.$colpick=self.$colpickTemplate.clone().removeClass('usof-colpick-template');self.$colpick.insertAfter(self.$input);self.$curentColorBox=$('.first .usof-colpick-color',self.$colpick);self.$gradientColorBox=$('.second .usof-colpick-color',self.$colpick);self.$hueArr=$('.first .usof-colpick-hue-selector',self.$colpick);self.$hueArr2=$('.second .usof-colpick-hue-selector',self.$colpick);self.$alphaArr=$('.first .usof-colpick-alpha-selector',self.$colpick);self.$gradientAlphaArr=$('.second .usof-colpick-alpha-selector',self.$colpick);self.$alphaContainer=$('.first .usof-colpick-alpha',self.$colpick);self.$gradientAlphaContainer=$('.second .usof-colpick-alpha',self.$colpick);self.$hueContainer=$('.first .usof-colpick-hue',self.$colpick);self.$gradientHueContainer=$('.second .usof-colpick-hue',self.$colpick);self.$angleContainer=$('.usof-colpick-angle',self.$colpick);self.$angle=$('.usof-colpick-angle-selector',self.$colpick);self.$palette=$('.usof-colpick-palette',self.$colpick);self.$selector=$('.first .usof-colpick-color-selector',self.$colpick);self.$gradientDot=$('.second .usof-colpick-color-selector',self.$colpick);self.$switchers=$('.usof-colpick-palette + .usof-radio input[type="radio"]',self.$colpick);self.$switchersBox=$('.usof-radio',self.$colpick);if(!self.isValidColor(self.value)){return}
...
self._events={hide:self.hide.bind(self),setPosition:self.setPosition.bind(self),stop:self._stop.bind(self),upAlpha:self.upAlpha.bind(self),upAngle:self.upAngle.bind(self),upHue:self.upHue.bind(self),upSelector:self.upSelector.bind(self),};self._specEvents={};self.$hueContainer.off('mousedown touchstart').on('mousedown touchstart',function(e){e.preventDefault();self.downHue(e)});self.$gradientHueContainer.off('mousedown touchstart').on('mousedown touchstart',function(e){e.preventDefault();self.downHue(e,!0)});self.$curentColorBox.off('mousedown touchstart').on('mousedown touchstart',function(e){e.preventDefault();self.downSelector(e)});self.$gradientColorBox.off('mousedown touchstart').on('mousedown touchstart',function(e){e.preventDefault();self.downSelector(e,!0)});self.$alphaContainer.off('mousedown touchstart').on('mousedown touchstart',function(e){e.preventDefault();self.downAlpha(e)});self.$gradientAlphaContainer.off('mousedown touchstart').on('mousedown touchstart',function(e){e.preventDefault();self.downAlpha(e,!0)});self.$angleContainer.off('mousedown touchstart').on('mousedown touchstart',function(e){e.preventDefault();self.downAngle(e)});self.$colpick.css('display','flex');self.$palette.on('mousedown',function(e){self._stop(e);self.colorPalette(e)});self.setPosition();self.$document.on('scroll',self._events.setPosition);self.$window.on('resize',self._events.setPosition);self.$switchers.removeAttr('checked').filter('[value="'+self.colors.state+'"]').prop('checked','checked');self.$document.off('change','.usof-colpick-palette + .usof-radio input[type="radio"]').on('change','.usof-colpick-palette + .usof-radio input[type="radio"]',function(e){self._stop(e);var value=$(e.target).closest('input').val();self.toggleGradient(value,!0)});self.$input.off('blur').on('blur',self._events.hide);self.$switchersBox.off('mousedown').on('mousedown',self._events.stop);self.timeout=setTimeout(function(){self.$input.select()},5)},_stop:function(e){e.preventDefault();e.stopPropagation()},colorPalette:function(e){var $target=$(e.target),color,colorId,currId,matches,max,palette,self=this,state='solid';if($target.is('span')){color=$target.attr('style');if(matches=/^[^:]*:([\s\S]*)$/.exec(color)){if(self.isValidColor(matches[1])){self.$input.val(matches[1]);if(self.withGradient){state=self.isGradient(matches[1])?'gradient':'solid'}
self.toggleGradient(state,!0);self.$switchers.filter('[value="'+self.colors.state+'"]').prop('checked','checked')}}}
if(!$('.usof-form').length){return!1}
self.paletteSend=function(data){if(!data||self.sending){return}
self.sending=!0;$.ajax({type:'POST',url:$usof.ajaxUrl,dataType:'json',data:{action:'usof_color_palette',color:JSON.stringify(data),_wpnonce:$('.usof-form [name="_wpnonce"]').val(),_wp_http_referer:$('.usof-form [name="_wp_http_referer"]').val()},success:function(result){$('.usof-colpick-palette').html(result.data.output);self.sending=!1}})};if($target.hasClass('usof-colpick-palette-add')){$target.addClass('adding');palette={value:self.$input.val()};max=self.$palette.children('.usof-colpick-palette-value').length;if(max<8){self.paletteSend(palette)}}
if($target.hasClass('usof-colpick-palette-delete')){currId=$.inArray($target.closest('.usof-colpick-palette-value')[0],$('.usof-colpick-palette-value',self.$palette));if(currId>=0){colorId={colorId:currId};self.paletteSend(colorId);$target.closest('.usof-colpick-palette-value').addClass('deleting')}}},setPosition:function(){var self=this,coordinates=self.$input.offset(),bottomSpace=_document.body.clientHeight-(coordinates.top-_window.pageYOffset),calWrapH=self.$colpick.outerHeight(),top=self.colors.inputHeight,right='auto';if(bottomSpace<calWrapH){top=-calWrapH}
...
self.colors.state=state;self.$switchers.filter('[value="'+self.colors.state+'"]').prop('checked','checked');self.change(gradient,setColor)},hide:function(){var self=this;self.$colpick.css('display','none').removeClass('type_gradient');if(self.timeout){clearTimeout(self.timeout)}
var value=self.$input.val();if(self.colors.initialColor!=value){self.$input.trigger('change')}
self.$hueContainer.off('mousedown touchstart');self.$gradientHueContainer.off('mousedown touchstart');self.$curentColorBox.off('mousedown touchstart');self.$gradientColorBox.off('mousedown touchstart');self.$alphaContainer.off('mousedown touchstart');self.$gradientAlphaContainer.off('mousedown touchstart');self.$angleContainer.off('mousedown touchstart');self.$palette.off('mousedown');self.$switchers.off('change');self.$input.off('blur').trigger('blur');self.$colpick.remove();self.$input[0].selectionStart=value.length;self.$input[0].blur();self._inited=!1},isValidColor:function(value){var gradient,valueG2,valueG1,self=this;if(typeof value=='string'){if(self.colorNameToHex(value)){value=self.hexToHsba(self.colorNameToHex(value))}else if(self.isGradient(value)){gradient=self.gradientParser(value);value=self.hexToHsba(gradient.hex);self.colors.gradient=gradient}else if(value=='transparent'){value={h:360,s:0,b:0,a:0,}}else{value=self.hexToHsba(self.normalizeHex(value))}}else if(value.r!=_undefined&&value.g!=_undefined&&value.b!=_undefined){value=self.rgbaToHsba(value)}else{return!1}
...
if(self.colors.initialColor!=='color'){self.$input.val(color).trigger('change')}},};$.usof_colorPicker={colorNameToHex:USColorPicker.colorNameToHex,gradientParser:USColorPicker.gradientParser,hexToRgba:USColorPicker.hexToRgba,hide:USColorPicker.hide,isGradient:USColorPicker.isGradient,normalizeHex:USColorPicker.normalizeHex,rgbaToHex:USColorPicker.rgbaToHex};$.fn.usof_colorPicker=function(options){return USColorPicker.init(options)}}(jQuery);!function($,undefined){var _window=window,_document=document;if(_window.$usof===undefined){return}
...
$usof.field.color={init:function(options){var self=this;self.$color=$('.usof-color',self.$row);self.$clear=$('.usof-color-clear',self.$color);self.$list=$('.usof-color-list',self.$color);self.$preview=$('.usof-color-preview',self.$color);self.withGradient=self.$color.hasClass('with-gradient');self.isDynamicСolors=self.$color.hasClass('dynamic_colors');self._toggleInputColor(self.getColor());self.$input.off('click').on('click',self._events.initColpick.bind(self)).on('input',self._events.inputValue.bind(self)).on('change',self._events.changeValue.bind(self));self.$clear.on('click',self._events.inputClear.bind(self));if(self.isDynamicСolors){self.$color.on('click','.usof-color-arrow',self._events.toggleList.bind(self)).on('click','.usof-color-list-item',self._events._changeColorListItem.bind(self))}
$(_document).mouseup(self._events.hideList.bind(self))},_events:{initColpick:function(){var self=this;self.$input.usof_colorPicker({input:self.$input,value:self.getColor(),onChange:function(colors){self._invertInputColors(colors.color.first.rgba)}})},toggleList:function(e){var self=this;if(!self.$color.is('.show')){self.initDynamicColors()}
self.$color.toggleClass('show')},_changeColorListItem:function(e){this.сhooseColorVar($(e.currentTarget).data('name')||'')},hideList:function(e){var self=this;if(!self.$color.hasClass('show')){return}
if(!self.$color.is(e.target)&&self.$color.has(e.target).length===0){self.$color.removeClass('show')}},inputValue:function(){var self=this,value=self.getValue();if(value.indexOf('_')!==-1){self.initDynamicColors()}},changeValue:function(){var self=this,value=self.getValue();if(value.indexOf('_')!==-1){$('[data-name^="'+value+'"]:first',self.$list).trigger('click')}else{self.setValue(value);self.trigger('change',value)}},inputClear:function(){var self=this;if(self.$color.hasClass('show')){self.$color.removeClass('show')}
self.setValue('')}},сhooseColorVar:function(name,quiet){var self=this,$target=$('[data-name="'+name+'"]:first',self.$list),value=$target.data('value')||'';$('[data-name]',self.$list).removeClass('selected');$target.addClass('selected');self.$preview.css('background',value);self.$input.val($target.data('name')||'');if(!quiet){self.trigger('change',self.$input.val())}
self._toggleInputColor(value);self.$color.removeClass('show')},initDynamicColors:function(){var self=this;if(self.$color.hasClass('list-inited')){return}
var insertItem=function($el,item){if(self.name===item.name){return}
var $item=$('<div></div>'),$palette=$('<div class="usof-colpick-palette-value"><span></span></div>'),value=self.getValue();$palette.find('span').css('background',item.value).attr('title',item.value);$item.addClass('usof-color-list-item').attr('data-name',item.name).data('value',item.value).append($palette).append('<span class="usof-color-list-item-name">'+item.title+'</span>');if(value.indexOf('_')!==-1&&item.name===value){$item.addClass('selected')}
$el.append($item)};$.each($usof.getData('dynamicColors')||{},function(key,item){if($.isArray(item)&&item.length){$group=$('> [data-group="'+key+'"]:first',self.$list);if(!$group.length){$group=$('<div class="usof-color-list-group" data-group="'+key+'"></div>');self.$list.append($group)}
$.each(item,function(_,_item){insertItem.call(self,$group,_item)})}else{insertItem.call(self,self.$list,item)}});self.$color.addClass('list-inited')},setValue:function(value,quiet){var self=this;value=value.trim();if(value.indexOf('_')!==-1){self.initDynamicColors();self.сhooseColorVar(value,quiet);return}
var r,g,b,a,hexR,hexG,hexB,gradient,rgba={};self.convertRgbToHex=function(color){if(m=/^([^0-9]{1,3})*(\d{1,3})[^,]*,([^0-9]{1,3})*(\d{1,3})[^,]*,([^0-9]{1,3})*(\d{1,3})[\s\S]*$/.exec(color)){rgba={r:m[2],g:m[4],b:m[6],};hexR=m[2]<=255?("0"+parseInt(m[2],10).toString(16)).slice(-2):'ff';hexG=m[4]<=255?("0"+parseInt(m[4],10).toString(16)).slice(-2):'ff';hexB=m[6]<=255?("0"+parseInt(m[6],10).toString(16)).slice(-2):'ff';color='#'+hexR+hexG+hexB;return color}};if($.usof_colorPicker.isGradient(value)){gradient=$.usof_colorPicker.gradientParser(value);rgba=$.usof_colorPicker.hexToRgba(gradient.hex)}else if((m=/^[^,]*,[^,]*,[\s\S]*$/.exec(value))){if(m=/^[^,]*(,)[^,]*(,)[^,]*(,)[^.]*(\.|0)[\s\S]*$/.exec(value)){if(m[4]==='.'||m[4]==0){if(m=/^([^0-9]{1,3})*(\d{1,3})[^,]*,([^0-9]{1,3})*(\d{1,3})[^,]*,([^0-9]{1,3})*(\d{1,3})[^,]*,[^.]*.([^0-9]{1,2})*(\d{1,2})[\s\S]*$/.exec(value)){rgba={r:m[2],g:m[4],b:m[6],};r=m[2]<=255?m[2]:255;g=m[4]<=255?m[4]:255;b=m[6]<=255?m[6]:255;a=m[8];value='rgba('+r+','+g+','+b+',0.'+a+')'}}else{value=self.convertRgbToHex(value)}}else{value=self.convertRgbToHex(value)}}else{if(m=/^\#?[\s\S]*?([a-fA-F0-9]{1,6})[\s\S]*$/.exec(value)){if(value=='inherit'||value=='transparent'||$.usof_colorPicker.colorNameToHex(value)){value=value}else{value=$.usof_colorPicker.normalizeHex(m[1]);rgba=$.usof_colorPicker.hexToRgba(value)}}}
if(value==''){self.$preview.removeAttr('style');self.$input.removeClass('with_alpha')}else{if(value=='inherit'||value=='transparent'){self.$input.removeClass('white');self.$preview.css('background',value)}else if(gradient){if(self.withGradient){self.$preview.css('background',gradient.gradient);self.$input.val(gradient.gradient)}else{value=gradient.hex;self.$preview.css('background',value);self.$input.val(value)}}else{self.$preview.css('background',value);self.$input.val(value)}}
if(value==''||value=='inherit'||value=='transparent'){self.$input.removeClass('white')}else{self._invertInputColors(rgba)}
self.parentSetValue(value,quiet)},getValue:function(){return $.trim(this.$input.val())||''},getColor:function(){var self=this,value=self.getValue();if(value.indexOf('_')!==-1){var itemValue=$('[data-name="'+value+'"]:first',self.$list).data('value')||'';value=itemValue||self.$color.data('value')||value}
return $.trim(value)},_toggleInputColor:function(value){var self=this;if(!value){self.$input.removeClass('white');return}
if(value.slice(0,1)==='#'&&value.length===4){value=value.replace(/^#([\dA-f])([\dA-f])([\dA-f])$/,"#$1$1$2$2$3$3")}
if(value!=='inherit'&&value!=='transparent'&&value.indexOf('linear-gradient')===-1){if($.usof_colorPicker.colorNameToHex(value)){self._invertInputColors($.usof_colorPicker.hexToRgba($.usof_colorPicker.colorNameToHex(value)))}else{self._invertInputColors($.usof_colorPicker.hexToRgba(value))}}else if(value.indexOf('linear-gradient')!==-1){var gradient=$.usof_colorPicker.gradientParser(value);if(gradient!=!1){self._invertInputColors($.usof_colorPicker.hexToRgba(gradient.hex))}}},_invertInputColors:function(rgba){if(!rgba&&(typeof rgba!='object')){return}
...
this.$input[light<178?'addClass':'removeClass']('white')}}}(jQuery);!function($,undefined){var _window=window;_window.$ush=_window.$ush||{};if($ush.isUndefined(_window.$usof)){return}
...
self.$select.val(value);self._setEditLink(value);self.$container.attr('selected-value',value)},_setEditLink:function(value){var self=this,defaultLayout=$('option:selected',self.$select).data('default-layout');if(!isPlainObject(defaultLayout)){defaultLayout={}}
...
$addDesc[isNumericValue?'addClass':'removeClass']('hidden');$editLink[isNumericValue?'removeClass':'addClass']('hidden')},setValue:function(value,quiet){var self=this;self.parentSetValue(''+value);self._syncSelected();if(!quiet){self.$input.trigger('change')}}}}(jQuery);!function($,undefined){var _window=window,_document=document;if(_window.$usof===undefined){return}
$usof.field.font={init:function(options){this.parentInit(options);this.$preview=this.$row.find('.usof-text-preview');this.$weightsContainer=this.$row.find('.usof-checkbox-list');this.$weightCheckboxes=this.$weightsContainer.find('.usof-checkbox');this.$weights=this.$weightsContainer.find('input');this.previewParams=this.$row.find('.usof-preview-params-json')[0].onclick()||{};this.notLoadedFonts=[];this.fontInited=!1;this.$fieldFontName={};var $autocomplete=$('.type_autocomplete',this.$row);this.fontsGroupKeys=$autocomplete[0].onclick()||{};$autocomplete.removeAttr('onclick');if($autocomplete.length){this.$fieldFontName=new $usof.field($autocomplete);this.$fieldFontName.trigger('beforeShow')}
...
var self=this,fontsTimeoutId=setTimeout(function fontsTimeout(){if($usof.googleLoaded){self.fonts=$usof.googleFonts;self._init();clearTimeout(fontsTimeoutId)}else{fontsTimeoutId=setTimeout(fontsTimeout,500)}},500)},_init:function(){var self=this;var initNotLoadedFonts=function(){['websafe','uploaded'].map(function(groupName){if(self.fontsGroupKeys.hasOwnProperty(groupName)){$('[data-group="'+self.fontsGroupKeys[groupName]+'"] > *',self.fontsGroupKeys.$list).each(function(_,item){var value=$(item).data('value')||'';if(value&&$.inArray(value,self.notLoadedFonts||[])===-1){self.notLoadedFonts.push(value)}})}})};initNotLoadedFonts();if(this.curFont&&$.inArray(this.curFont,['get_h1','none'])===-1&&$.inArray(this.curFont,this.notLoadedFonts||[])===-1){$('head').append('<link href="//fonts.googleapis.com/css?family='+this.curFont.replace(/\s+/g,'+')+'" rel="stylesheet" class="usof_font_'+this.name+'" />');this.$preview.css('font-family',this.curFont+'')}else if(this.curFont!='none'&&$.inArray(this.curFont,this.notLoadedFonts||[])!==-1){this.$preview.css('font-family',this.curFont+'')}
...
fontName=parts[0]||'none';fontWeights=parts[1]||'400,700';fontWeights=fontWeights.split(',');if(fontName!=this.curFont){$('link[data-usof-for="'+this.name+'"]').remove();if(fontName=='none'){this.$preview.css('font-family','')}else if($.inArray(fontName,this.notLoadedFonts||[])!==-1){var prepareFontName=fontName;if(this.fontHasGroup(prepareFontName,'uploaded')){prepareFontName='"'+prepareFontName+'"'}
this.$preview.css('font-family',prepareFontName)}else{if(this.curFont!=='get_h1'){$('head').append('<link href="//fonts.googleapis.com/css?family='+fontName.replace(/\s+/g,'+')+'" rel="stylesheet" data-usof-for="'+this.name+'" />')}
...
if(this.previewParams.fontweight==undefined){this.$preview.css('font-weight',fontWeights.length?parseInt(fontWeights[0]):'')}
...
return fontName+'|'+fontWeights.join(',')}}}(jQuery);!function($,undefined){$usof.field.typography_options={init:function(options){var self=this;self._events={changePropField:$ush.debounce(self._changePropField.bind(self),1),switchSection:self._switchSection.bind(self),};self.fields={};self._$fontWeights=[];self._usbParams=self.$row.data('usb-params')||{};self._notExistsText='';var $notExistsText=$('.us-font-weight-not-exists-text:first',self.$row);if($notExistsText.length){self._notExistsText=$notExistsText.text();$notExistsText.remove()}
self.$head=$('head');self.$title=$('> .usof-form-row-title',self.$row);self.$typographyVars=$('#us-typography-vars');$('.usof-form-row-control:first [data-name]',self.$row).each(function(_,node){var $node=$(node),usofField=$node.data('usofField');if(usofField instanceof $usof.field){var name=$node.data('name');usofField.on('change',self._events.changePropField).on('syncResponsiveState',function(field,screenName){if(self.isEditLive()&&$usb.find('preview')){$usb.preview.fieldSetResponsiveScreen(screenName)}
if(!self.isEditLive()){$ush.debounce_fn_1ms(self._renderFontPreview.bind(self));$.each(self.fields,function(name,_field){if(field.name!==name){_field._usbSyncResponsiveState(_field,screenName)}})}});self.fields[name]=usofField;if(usofField.type==='select'&&['font-weight','bold-font-weight'].indexOf(name)>-1){$('option',usofField.$row).each(function(_,node){if($ush.toString(node.value).indexOf('var(')===0){return}
$node=$(node);$node.data('text',$node.text());self._$fontWeights.push($node)})}}});self.$row.on('click','> .usof-form-row-title',self._events.switchSection);if(!self.isEditLive()){self.on('afterShow',function(){self._setFontProperties(self.getValue())})}},_switchSection:function(e){var self=this;self.$row.toggleClass('expand',!self.$row.hasClass('expand')).siblings().removeClass('expand')},_syncAvailableWeights:function(fontFamily){var self=this;for(var i in self._$fontWeights){var $node=self._$fontWeights[i];if($node.hasClass('not-exists')){$node.text($node.data('text')).removeClass('not-exists')}}
...
self.hasAjaxHandler=!$.isEmptyObject(self.options.ajax_data);self.$container.on('click','.usof-autocomplete-list [data-value]',self._events.selectedValue);if(self.isMultiple()){self.$container.on('click','.usof-autocomplete-selected button',self._events.removeSelectedValue)}else{self.$container.on('click','.usof-autocomplete-selected:first',self._events.showList)}
...
self.clearMessage();self.$container.addClass('loading');var valueList=$ush.toString(self.getValue()).split(valueSeparator);_$temp.xhr=$.ajax({data:data,dataType:'json',url:ajaxurl,cache:!1,success:function(res){if(!res.success){self._showMessage.call(self,res.data.message);return}
...
self.$options.append(`<div class="usof-autocomplete-selected">
				...
				</div>`);$('[data-value="'+value+'"]:not(.selected)',self.$list).addClass('selected')},_initSelectedValue:function(values){var self=this;if(self.isMultiple()){self._loadSelectedValues(values)}else{self._showSelectedValue(values,$('[data-value="'+values+'"]',self.$list).text())}},_selectedValue:function(e){var self=this,$target=$(e.currentTarget);if($target.hasClass('selected')&&self.isMultiple()){self._removeSelectedValue(e);return}
...
self._clearList();self.getItems(function(items){if($.isEmptyObject(items)){self.showMessage(self.options.no_results_found)}})},showMessage:function(text){var self=this;self.$list.addClass('hidden');self.$message.text(text).removeClass('hidden')},clearMessage:function(){var self=this;self.$list.removeClass('hidden');self.$message.addClass('hidden').text('')},getValue:function(){var self=this;return(self.$value instanceof $)?self.$value.val():''},setValue:function(value,quiet){var self=this;self.$value.val(value);self._initSelectedValue(value);if(!quiet){self.trigger('change',[value])}}}}(jQuery);!function($,undefined){var _window=window,_undefined=undefined;if(_window.$usof===_undefined){return}
...
if(!isUndefined(data.url)){self.$inputUrl.val(data.url)}
...
$usof.field.text={init:function(){var self=this;self._dynamicLabels={};self.$text=$('input[type=text]',self.$row);self._events={changeField:$ush.debounce(self._changeField.bind(self)),setExampleValue:self._setExampleValue.bind(self),syncCurrentValue:self._syncCurrentValue.bind(self),};self.initDynamicValue();self.$row.on('click','.usof-example',self._events.setExampleValue).on('change paste keyup','input[type=text]',self._events.changeField);if(self.hasResponsive()){self.on('setResponsiveState',self._events.syncCurrentValue)}},_setExampleValue:function(e){var self=this,exampleValue=($(e.target).closest('.usof-example').html()||'');self.$text.val(exampleValue);self.setCurrentValue(exampleValue)},_changeField:function(e){this.setCurrentValue(e.currentTarget.value)},_syncCurrentValue:function(){var self=this;self.$text.val(self.getCurrentValue())},setValue:function(value,quiet){var self=this;self.parentSetValue(''+value);self._syncCurrentValue();if(self.popupId){self.setDynamicValue(value)}}};$.extend($usof.field.text,{initDynamicValue:function(){var self=this;$.extend(self._events,{selectDynamicVariable:self._selectDynamicValue.bind(self),removeDynamicValue:self._removeDynamicValue.bind(self),})
...
$usof.field.design_options={init:function(){var self=this;self.defaultGroupValues={};self.defaultValues={};self.groupParams={};self._lastSelectedScreen='default';self.$container=$('.usof-design-options',self.$row);self.$input=$('textarea.usof_design_value',self.$container);self.$import=$('.usof-design-options-import',self.$container);self.$importHeader=$('.usof-design-options-import-header',self.$import);self.$importFooter=$('.usof-design-options-import-footer',self.$import);self.$importBtnCopy=$('button[data-action="copy"]',self.$importHeader);self.states=self.$container[0].onclick()||['default'];self.extStates=self.states.slice(1);self.$container.removeAttr('onclick');self.isWPBakery=self.$input.hasClass('wpb_vc_param_value');self.hasStringValue=(self.isWPBakery||self.isEditLive());if(self.isWPBakery){self.$container.closest('.edit_form_line').addClass('usof-not-live')}
self.__checkChangeValues=$ush.debounce(self.checkChangeValues.bind(self),1);if(self.extStates.length){$('[data-responsive-state-content="default"]',self.$container).each(function(_,content){var $content=$(content);self.extStates.map(function(screen){var $cloneContent=$content.clone().attr('data-responsive-state-content',screen).addClass('hidden');$content.after($cloneContent)})})}
...
field.screen=screen}});$.each(groupParams.fields,function(_,field){var $row=field.$row;if($row.attr('onclick')){field._data=$row[0].onclick()||'';$row.removeAttr('onclick');if(field._data.hasOwnProperty('relations')){$row.append('<i class="fas fa-unlink"></i>').on('click','i.fas',self._events.watchAttrLink.bind(self,field))}}
field.trigger('beforeShow').on('change',$ush.debounce(self._events.changeValue.bind(self),1))})});var pid=setTimeout(function(){if(!self.inited){self.setValue(self.$input.val());self.checkChangeValues.call(self)}
self.$importBtnCopy.prop('disabled',!self.getValue());clearTimeout(pid)},1);$('.usof-responsive-buttons',self.$container).toggleClass('hidden',!self.extStates.length);self.$container.on('click','[data-accordion-id]',self._events.toggleAccordion.bind(self)).on('click','.usof-design-options-reset',self._events.resetValues.bind(self)).on('click','.usof-design-options-responsive',self._events.toggleResponsive.bind(self)).on('click','[data-responsive-state]',self._events.selectResponsiveState.bind(self));self.$import.on('click','button[data-action]',self._events.importActions.bind(self))},_events:{changeValue:function(field){var self=this,result={},enabledResponsives=[],valuesChanged={};$('[data-accordion-id].responsive',self.$container).each(function(_,node){var accordionId=$(node).data('accordionId'),props=self.defaultGroupValues[accordionId].default||{};for(var k in props){enabledResponsives.push(k);valuesChanged[k]=[];$.each(self.groupParams,function(_,groupParams){valuesChanged[k].push(groupParams.getValue(k))})}});$.each(self.groupParams,function(screen,groupParams){$.each(groupParams.getValues(),function(param,value){var defaultValue=self.defaultValues[screen][param],isValueChanged=!1;if(enabledResponsives.indexOf(param)>-1){for(var i in valuesChanged[param]){isValueChanged=valuesChanged[param][i]!==defaultValue;if(isValueChanged){break}}}
...
self.__checkChangeValues();self.$importBtnCopy.prop('disabled',!result);self.trigger('changeDesignField',field)},resetValues:function(e){e.stopPropagation();var self=this,$target=$(e.currentTarget),$groupHeader=$target.closest('[data-accordion-id]'),groupName=$groupHeader.data('accordion-id');if($groupHeader.hasClass('responsive')){self._events.toggleResponsive.call(self,e)}
...
var pid=setTimeout(function(){$groupHeader.removeClass('changed');clearTimeout(pid)},1000*0.5)},watchAttrLink:function(field,e,state){var self=this,$target=$(e.currentTarget),isUnlink=$target.hasClass('fa-unlink'),relations=[];if(state!==_undefined){isUnlink=state}
...
$target.toggleClass('fa-link',isUnlink).toggleClass('fa-unlink',!isUnlink);if(relations.length){relations.map(function(item){item.$input.prop('disabled',isUnlink)});field.watchValue=isUnlink;if(isUnlink){field.$input.focus().on('input',self._events.changeRelationsValue.bind(self,relations)).trigger('input')}else{field.$input.off('input')}}},changeRelationsValue:function(fields,e){var value=$(e.currentTarget).val();fields.map(function(item){if(item instanceof $usof.field){item.setValue(value)}})},toggleAccordion:function(e){var $target=$(e.currentTarget),$content=$('[data-accordion-content="'+$target.data('accordion-id')+'"]');if($target.hasClass('active')){$target.removeClass('active');$content.removeClass('active')}else{$target.siblings().removeClass('active');$content.siblings().removeClass('active');$target.addClass('active');$content.addClass('active')}},toggleResponsive:function(e){e.preventDefault();e.stopPropagation();var self=this,$target=$(e.currentTarget),$header=$target.closest('[data-accordion-id]'),groupKey=$header.data('accordion-id'),isEnabled=$header.hasClass('responsive'),isFirstResponsive=!isEnabled?!$('.usof-design-options-header.responsive:first',self.$container).length:!1;$header.toggleClass('responsive',!isEnabled);if(!isEnabled){if(isFirstResponsive){self.trigger('syncResponsiveState',self._lastSelectedScreen)}
self.switchResponsiveState(self._lastSelectedScreen)}else{self.switchResponsiveState('default',!0)}
if(self.defaultGroupValues.hasOwnProperty(groupKey)){self.extStates.map(function(screen){var values=$.extend({},self.defaultGroupValues[groupKey][screen]||{});if(!isEnabled){$.each(values,function(prop){if(self.groupParams['default'].fields.hasOwnProperty(prop)){values[prop]=self.groupParams['default'].fields[prop].getValue()}})}
...
if(!isEnabled&&self.groupParams.hasOwnProperty(screen)){$.each(self.groupParams['default'].fields,function(_,field){if(field.hasOwnProperty('watchValue')){$('.fas',self.groupParams[screen].fields[field.name].$row).trigger('click',field.watchValue)}})}})}},importActions:function(e){var self=this,$target=$(e.target),action=''+$target.data('action');if(action=='copy'){self._copyValueToClipboard()}else if(action=='paste'){self.$import.addClass('show_input');self.$input[0].select()}else if(action=='cancel'){self.$import.removeClass('show_input show_novalid')}else if(action=='apply'){var value=self.$input.val();if(!value){return}
var isValidValue=self._isValidValue(value);self.$import.toggleClass('show_novalid',!isValidValue);if(!isValidValue){self.$input.val('');return}
$.each(self.groupParams,function(screen,groupParams){groupParams.setValues(self.defaultValues[screen]||{},!0)});self.setValue(value);self.$import.removeClass('show_input');self.$importBtnCopy.prop('disabled',!value)}},selectResponsiveState:function(e){var self=this,screen=$(e.currentTarget).data('responsive-state');self.switchResponsiveState(screen);self.trigger('syncResponsiveState',screen);self._lastSelectedScreen=screen}},_usbSyncResponsiveState:function(_,state){var self=this;self._lastSelectedScreen=state||'default';self.switchResponsiveState(self._lastSelectedScreen)},switchResponsiveState:function(screen,hidden){var self=this;if(!screen){return}
var hasResponsiveClass=!hidden?'.responsive':':not(.responsive)'
var $target=$('[data-accordion-id]'+hasResponsiveClass,self.$container).next('[data-accordion-content]').find('[data-responsive-state="'+screen+'"]');$target.siblings().removeClass('active');$target.addClass('active').closest('.usof-design-options-content').find('> [data-responsive-state-content]').addClass('hidden').filter('[data-responsive-state-content="'+screen+'"]').removeClass('hidden')},_isValidValue:function(value){var self=this;try{value=$ush.toPlainObject((''+value).trim());for(var i in self.states){var state=self.states[i];if(state&&!!value[state]){return!0}}}catch(err){}
return!1},checkChangeValues:function(){var self=this,currentGroupValues={};$.each(self.groupParams,function(screen,groupParams){$.each(groupParams.fields,function(_,field){var groupName=field.$row.closest('[data-accordion-content]').data('accordion-content');if(!currentGroupValues.hasOwnProperty(groupName)){currentGroupValues[groupName]={}}
if(!currentGroupValues[groupName].hasOwnProperty(screen)){currentGroupValues[groupName][screen]={}}
currentGroupValues[groupName][screen][field.name]=field.getValue()})});$.each(self.defaultGroupValues,function(groupName,devices){var change=!1;$.each(devices,function(screen,values){if(!currentGroupValues.hasOwnProperty(groupName)||!currentGroupValues[groupName].hasOwnProperty(screen)){return}
...
var pid=setTimeout(function(){$.each(self.groupParams,function(screen,groupParams){if(!self.hasStringValue){groupParams.setValues(self.defaultValues[screen]||{},!0)}
...
groupParams.setValues(values,!0);$.each(groupParams.fields,function(_,field){if(field.hasOwnProperty('_data')&&field._data.hasOwnProperty('relations')){var $row=field.$row,value=$.trim(field.getValue()),isLink=[];(field._data.relations||[]).map(function(name){if(value&&self.groupParams[field.screen].fields.hasOwnProperty(name)){isLink.push(value===$.trim(self.groupParams[field.screen].fields[name].getValue()))}});if(isLink.length){isLink=isLink.filter(function(v){return v==!0});if(isLink.length===3){var pid=setTimeout(function(){$('i.fas',$row).trigger('click');clearTimeout(pid)},1)}}}})});var responsiveGroups={};self.extStates.map(function(screen){var values=savedValues[screen]||{};$.each(self.defaultGroupValues,function(groupKey,devices){var isEnable=!1;$.each(devices[screen],function(prop){if(!responsiveGroups[groupKey]){responsiveGroups[groupKey]=values.hasOwnProperty(prop)}})})});$.each(responsiveGroups,function(groupKey,isEnable){$('[data-accordion-id="'+groupKey+'"]',self.$container).toggleClass('responsive',isEnable)});self.checkChangeValues.call(self);self.switchResponsiveState('default',!0);clearTimeout(pid)},1);if(value){value=self.hasStringValue?value:$ush.toString(value)}
self.$input.val(''+value);if(!quiet){self.trigger('change',[value])}
if(!self.$input.hasClass('wpb_vc_param_value')){$('> div',self.$container).removeClass('active')}},_copyValueToClipboard:function(){var self=this;if(!self.$input.val()){return}
self.$input[0].select();_document.execCommand('copy');if(_window.getSelection){_window.getSelection().removeAllRanges()}else if(_document.selection){_document.selection.empty()}},forceWPBValue:function(){var self=this;if(self.hasStringValue){self.setValue(self.getValue())}}}}(jQuery);!function($,undefined){var _window=window,_document=document;if(_window.$usof===undefined){return}
...
if(self._getColorValue('color_text').indexOf('gradient')!==-1){color=$.usof_colorPicker.gradientParser(self._getColorValue('color_text')).hex;style.default+='color:'+color+' !important;'}else{self.$btn.css('color',self._getColorValue('color_text'))}
...
var colorHover;if(self._getColorValue('color_text_hover').indexOf('gradient')!==-1){colorHover=($.usof_colorPicker.gradientParser(self._getColorValue('color_text_hover'))).hex}else{colorHover=self._getColorValue('color_text_hover')}
